...
class ChatRoomRepositoryImpl(database: MongoDatabase) : ChatRoomRepository {
    ...
    private val collectionChatRooms: MongoCollection<Document> =
        database.getCollection("chat_rooms")
    ...
    override suspend fun addUserToChatRoom(chatRoomId: String, userId: String): Flow<Resource<Boolean>> = flow {
        ...
        try {
            if (userId.isBlank()) {
                emit(Resource.Error("El userId está vacío"))
                return@flow
            }
            ...
            if (userId in currentUsers) {
                emit(Resource.Success(false)) // El usuario ya existe
                return@flow
            }

            val update = collectionChatRooms.updateOne(
                Document("_id", ObjectId(chatRoomId)),
                Document("\$push", Document("users", userId))
            )

            if (update.modifiedCount > 0) {
                emit(Resource.Success(true))
            } else {
                emit(Resource.Error("No se pudo agregar el usuario a la sala"))
            }
        } catch (e: Exception) {
            e.printStackTrace()
